...
const elements = {
    ...
    mobileMorePopup: document.getElementById('mobile-more-popup'), // Added
...
function toggleMoreMenu() {
    ...
    const btn = document.getElementById('mob-btn-more');
    // Toggle active styling for the More button itself based on visibility
    if (!elements.mobileMorePopup.classList.contains('hidden')) {
        btn.classList.add('text-purple-500', 'text-white');
        btn.classList.remove('text-zinc-400');
    } else {
        // Only remove active style if we aren't "in" one of the more sections technically (though for popup typically we treat it as transient)
        btn.classList.remove('text-purple-500', 'text-white');
        btn.classList.add('text-zinc-400');
    }
}
...
function updateWatchlistButton(isInList) {
    ...
    if(isInList) {
        ...
        elements.watchlistBtnIcon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>'; // Checkmark
        // Active state: White background, Black text, White border
        elements.detailsWatchlistBtn.classList.add('bg-white', 'text-black', 'border-white');
    } else {
        elements.watchlistBtnText.textContent = "My List";
        elements.watchlistBtnIcon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>'; // Plus
        // Inactive state: Transparent background, White text, Zinc border
...
function switchType(type) {
    ...
    // Close Popup if open
    elements.mobileMorePopup.classList.add('hidden');
    ...
    // For "more" items, the "More" button stays colored
...
function playContent(item, season = null, episode = null) {
    ...
    // Hide UI elements when player opens
    ...
    // FIX: Fallback title if undefined
    ...
    if (source.name === "SuperEmbed") {
        ...
        // Custom format: embed?type={series/movie}&id={id}
        ...
        // Standard Construction for Videasy, Vidsrc, etc.
...
function navigateEpisode(direction) {
    ...
    let currentS = history?.season || 1;
    let currentE = history?.episode || 1;
    if (direction === 1) currentE++; 
    else currentE--;
    if (currentE < 1) { showToast("Start of Season"); return; }
...
function closeVideoModal() {
    ...
    setTimeout(() => {
        ...
        // Show UI elements when player closes
